...
public class TrapTargetGroupGridAction extends ActionSupport {
  ...
  private static final Comparator<TrapTargetGroupDto> descrComparator
    = new Comparator<TrapTargetGroupDto>() {
        public int compare(TrapTargetGroupDto o1, TrapTargetGroupDto o2) {
  ...
  public String execute() {
    CriteriaBuilder cb = trapTargetGroupDao.getCriteriaBuilder();
    CriteriaQuery<TrapTargetGroup> cq = cb.createQuery(TrapTargetGroup.class);
    Root<TrapTargetGroup> r = cq.from(TrapTargetGroup.class);
    cq.select(r);
    ...
    for (TrapTargetGroup trapTargetGroup: trapTargetGroupDao.findByCriteria(cq)) {
